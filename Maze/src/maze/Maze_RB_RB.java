...
import maze.Maze_Kruskals_Astar.Connection;
import processing.core.PApplet;
import processing.core.PImage;
import processing.core.PShape;
import processing.core.PVector;

public class Maze_RB_RB extends PApplet {
	public static void main(String[] args) {
		PApplet.main(new String[] { "--present", "maze.Maze_RB_RB" });
	}
		...
		if (!gameOver && !reachedGoal()) {
			...
			if (remainingDistance() < totalDistance/2 && !changed) {
				...
				if (pathIndex < botPath.path.size())
					botPath.findPathRB(NodeAry[botPath.finalpath.get(pathIndex).nodeNo],currentNode);
				botPath.PrintPath();
				pathIndex=0;
				changed=true;
			}
			...
			if(botPresent){
				if (pathIndex < botPath.finalpath.size())
					bot.target = new PVector(NodeAry[botPath.finalpath.get(pathIndex).nodeNo].x,
							NodeAry[botPath.finalpath.get(pathIndex).nodeNo].y);
				botArrive.update();
			}
			score();
		}
...
//				textSize(circleSize*3);
	...
	public void allSetting(){
		...
		playerPath = new BacktrackAlgo(NodeAry[Goal]);
		botPath = new BacktrackAlgo(NodeAry[Start]);
		...
		if(botPresent){
			...
			botPath.findPathRB(NodeAry[initialBotPosition], currentNode);
			botPath.PrintPath();
			
			botArrive= new Arrive(bot,3,2,10);
		}
				
		playerPath.findPathRB(NodeAry[Start],NodeAry[Goal]);
		//playerPath.PrintPath();
		makeNodeStatusFalse();
		clearNOdeHistory();
...
//		System.out.println("mazeSize :"+mazeSize);
	...
	public void makeNodeStatusFalse(){
		for(int i=0;i<NodeAry.length;i++){
			NodeAry[i].status = false;
		}
	}
	...
	class BacktrackAlgo {
		...
		public void findPathRB(Node start,Node goal){
			this.en = goal;
			path.clear();
			findPath(start);
		...
		public boolean findPath(Node next){
			if(next.nodeNo==en.nodeNo){
				path.push(next);
				return true;
			}else{
				path.push(next);
				float absx = character.location.x-bot.location.x;
				float absy = character.location.y-bot.location.y;
				
				if(absx<0 && absy<0){
					if (next.top == true && next.topNode.status == false) {
						next.status = true;
						if(findPath(next.topNode))
						return true;
					}
					if (next.left == true && next.leftNode.status == false) { 
						next.status = true;
						if(findPath(next.leftNode))
						return true;
					} 
					if (next.bot == true && next.botNode.status == false) {
						next.status = true;
						if(findPath(next.botNode))
						return true;
					}  
					if (next.right == true && next.rightNode.status == false) {	
						next.status = true;
						if(findPath(next.rightNode))
						return true;
					} 
		...
		public void PrintPath(){
			...
			for(int i=0;i<finalpath.size()-1;i++){
				System.out.print(finalpath.get(i).nodeNo+" ");
			}
		}
		public void showPath(boolean bot) {
			...
			for(int i=0;i<finalpath.size()-1;i++){
				if(!bot)
					...
					stroke(0, 255, 0);
				line(finalpath.get(i).x, finalpath.get(i).y, finalpath.get(i+1).x, finalpath.get(i+1).y);
			}
		}
	...
	public class CreateConnection {
		...
		public void GeneratePath() {
			...
			st.push(NodeAry[0]);
			while (!st.isEmpty()) {
				selectRandomNode(st.peek());
			}
		}

		public void selectRandomNode(Node N) {
			Node randomNode, randomNode2;
			int checknode = 0;
			ArrayList<Node> list = new ArrayList<Node>();
			ArrayList<Node> list2 = new ArrayList<Node>();
			list = possibleNeighbour(N);
			if (N.leftNode != null) {
				list2.add(N.leftNode);
			}
			if (N.rightNode != null) {
				list2.add(N.rightNode);
			}
			if (N.topNode != null) {
				list2.add(N.topNode);
			}
			...
			if(st.size()>1){
				Node K=st.pop();
				if(list2.contains(st.peek())){
					list2.remove(st.peek());
				}
				st.push(K);
			}
			randomNode2 = list2.get(randomizer.nextInt(list2.size()));
			if (list.size() > 0) {
				randomNode = list.get(randomizer.nextInt(list.size()));
				st.add(randomNode);
				if (randomNode == N.leftNode) {
					N.left = true;
					randomNode.right = true;
				} else if (randomNode == N.rightNode) {
					N.right = true;
					randomNode.left = true;
				} else if (randomNode == N.topNode) {
					N.top = true;
					randomNode.bot = true;
				} else if (randomNode == N.botNode) {
					N.bot = true;
					randomNode.top = true;
				}


				Connection c = new Connection(N, randomNode);
				Connection c1 = new Connection(randomNode, N);// extra need to
				// remove
				conList.add(c);
				conList.add(c1);// extra
				check = 0;
			} else {
				if (check == 0) {
					int k = (int) random(0, 10);
					if (k< 7) {
						if (randomNode2 == N.leftNode) {
							N.left = true;
							randomNode2.right = true;
						} else if (randomNode2 == N.rightNode) {
							N.right = true;
							randomNode2.left = true;
						} else if (randomNode2 == N.topNode) {
							N.top = true;
							randomNode2.bot = true;
						} else if (randomNode2 == N.botNode) {
							N.bot = true;
							randomNode2.top = true;
						}
						Connection c = new Connection(N, randomNode2);

						Connection c1 = new Connection(randomNode2, N);// extra
						conList.add(c);
						conList.add(c1);// extra
					}
				}
		...
		public ArrayList possibleNeighbour(Node N) {
			ArrayList<Node> list = new ArrayList<Node>();
			if (N.leftNode != null && !visited.contains(N.leftNode) && !st.contains(N.leftNode)) {
				list.add(N.leftNode);
			}
			if (N.rightNode != null && !visited.contains(N.rightNode) && !st.contains(N.rightNode)) {
				list.add(N.rightNode);
			}
			if (N.topNode != null && !visited.contains(N.topNode) && !st.contains(N.topNode)) {
				list.add(N.topNode);
			}
			if (N.botNode != null && !visited.contains(N.botNode) && !st.contains(N.botNode)) {
				list.add(N.botNode);
			}
			return list;
		}
	...
	public class Astar{
		...
		public void findPath(Node Start, Node Goal,int st,int gl) {
			...
			while(itr.hasNext()){
				System.out.println(itr.next().nodeNo);
			}
		}
	...
	class Character {
		...
		public void getNewOrientation(PVector location2) {
			float tempOrient;
			//				System.out.println(ofinal+"aa");
			//				if((int)target.y != (int)location.y && (int)target.y != (int)location.y){
			//					System.out.println(ofinal);
	...
	public class Arrive {
		...
		public SteeringOutput getsteering() {
			...
			float targetSpeed;
			PVector targetVelocity;
			PVector direction = new PVector(myCharacter.target.x-myCharacter.location.x,myCharacter.target.y-myCharacter.location.y);
			float distance = direction.mag();
			...
			if(distance<targetRadius){
				if(myCharacter.bot){
					...
					if(i%10==0){
						try{
							if(pathIndex>=botPath.finalpath.size()){
								pathIndex = botPath.finalpath.size()-1;
							}
							makeNodeStatusFalse();
							//if(!changeBot){
							botPath.findPathRB(NodeAry[botPath.finalpath.get(pathIndex).nodeNo],currentNode);
					...
					myCharacter.velocity=new PVector(0,0);
					playerPath.findPathRB(currentNode,NodeAry[Goal]);
					makeNodeStatusFalse();
					i++;
				}
				return null;
		...
		void checkEdges() {
			if ((Math.abs(dist(myCharacter.location.x,myCharacter.location.y,myCharacter.target.x,myCharacter.target.y)) <= radiusofsat)) {// && Math.abs(target.y) <= radiusofsat
